...
const optimizedRoute = computed(() => {
  if (!props.tripItems || props.tripItems.length === 0)
    return []
  ...
  <div class="space-y-4">
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-4">
      <div class="mb-4 flex items-center justify-between">
        ...
        <button
          class="
            flex items-center gap-x-2 rounded-md bg-blue-600 px-3 py-2 text-sm
      ...
      <!-- 路線統計 -->
      <div class="mt-4 grid grid-cols-3 gap-4 border-t pt-4 text-center">
        <div>
          <div class="text-2xl font-bold text-blue-600">
            {{ optimizedRoute.filter(p => p.type === 'pickup').length }}
          </div>
        ...
        <div>
          <div class="text-2xl font-bold text-green-600">
            {{ optimizedRoute.filter(p => p.type === 'delivery').length }}
          </div>
          <div class="text-xs text-gray-500">
            送件點
          </div>
