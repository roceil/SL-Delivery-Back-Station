...
interface Order {
  ...
  luggageCount: number
  status: string
  pickupLocation: { id: string, name: string, address: string, area?: string }
  deliveryLocation: { id: string, name: string, address: string, area?: string }
  notes?: string
}
...
const availableOrders = computed(() => {
  ...
  return orders.value.filter(o =>
    o.status === 'confirmed' || form.value.selectedOrders.includes(o.id),
  )
})
...
const areaListModal = computed(() =>
  [...new Set(availableOrders.value.map(o => o.deliveryLocation?.area).filter(Boolean))].sort() as string[],
)
const destinationList = computed(() =>
  [...new Set(availableOrders.value.map(o => o.deliveryLocation?.name))],
)
...
const filteredAvailableOrders = computed(() => {
  return availableOrders.value.filter((o) => {
    if (modalKeyword.value) {
      ...
      if (!o.lineName.toLowerCase().includes(kw) && !(o.orderNumber ?? o.id).toLowerCase().includes(kw))
        return false
    }
    if (modalArea.value && o.deliveryLocation?.area !== modalArea.value)
      return false
...
const tempSelectedOrders = computed(() =>
  availableOrders.value.filter(o => tempSelected.value.includes(o.id)),
)
const tempTotalLuggage = computed(() =>
  tempSelectedOrders.value.reduce((sum, o) => sum + o.luggageCount, 0),
)
...
const selectedOrdersData = computed(() =>
  (orders.value ?? []).filter(o => form.value.selectedOrders.includes(o.id)),
)

const orderStats = computed(() => {
  const totalLuggage = selectedOrdersData.value.reduce((sum, o) => sum + o.luggageCount, 0)
  return { count: selectedOrdersData.value.length, totalLuggage }
})

const selectedCourier = computed(() =>
  (couriers.value ?? []).find(c => c.id === form.value.courierId),
)
...
const filteredStopOrders = computed(() => {
  if (!activeStop.value)
    return []
  const orderList = activeStopTab.value === 'pickup'
  ...
  return orderList.filter(o =>
    o.lineName.toLowerCase().includes(kw)
    || (o.orderNumber ?? o.id).toLowerCase().includes(kw),
  )
})
...
async function submitForm() {
  ...
  try {
    await $fetch(`/api/trips/${tripId}`, { method: 'PUT', body: form.value })
    router.push('/trips')
  }
  catch (error: any) {
    // eslint-disable-next-line no-alert
    alert(error.data?.message || '儲存行程失敗，請稍後再試')
  }
  finally {
                ...
                >
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    >
                      ...
                      <span class="text-neutral-900">{{ order.orderNumber ?? order.id }}</span>
                    </div>
                    <div
                      class="flex items-start gap-2 text-base tracking-[0.8px]"
                    >
          ...
          <div class="flex flex-col gap-2">
            ...
            >
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="shrink-0 text-xs tracking-wide text-neutral-600">
                  {{ order.orderNumber ?? order.id }}
                </span>
              </div>
              <!-- 第二行：旅客 · 行李 · 區域 -->
              <div class="mt-1 text-sm tracking-[0.7px] text-neutral-600">
                {{ order.lineName }} · {{ order.luggageCount }} 件行李
                <template v-if="order.deliveryLocation.area">
            ...
            <div
              v-if="filteredAvailableOrders.length === 0"
              class="
        ...
        <div class="col-span-4 flex flex-1 flex-col gap-4 p-4">
          ...
          <div class="flex flex-1 flex-col gap-2 overflow-y-auto">
            <div
              ...
              "
            >
              <span class="text-xs tracking-wide text-neutral-600">
                {{ order.orderNumber ?? order.id }}
              </span>
              <div
                class="
